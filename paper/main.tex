...
\usepackage{array} 
\usepackage{geometry}
\geometry{margin=2.5cm} % Pode alterar para 2.0cm se preferir margens ligeiramente menores

\begin{document}

\section{Methodology}

The proposed vacuum grasping algorithm evaluates point cloud data to determine the optimal Target Center Point (TCP) and approach vector for a multi-pad vacuum gripper. The method ensures physical feasibility and maximizes grasp stability. The pipeline consists of four main phases, as illustrated in Figure \ref{fig:pipeline_flowchart}.
\begin{figure}
    % O \makebox centraliza o excesso nos dois lados perfeitamente
    \makebox[\textwidth][c]{\includegraphics[width=1.04\textwidth, keepaspectratio]{images/Vacuum_Flow.pdf}}
    \caption{Overview of the proposed vacuum grasping algorithm pipeline, detailing the four main phases from point cloud preprocessing to the final target selection.}
...
\begin{table}
    ...
    \begin{tabularx}{\textwidth}{@{}llc >{\raggedright\arraybackslash}X@{}}
        ...
        Evaluation & Angular Seal Slices & $N_{slices} = 8$ & Number of radial sectors evaluated per suction cup. \\
