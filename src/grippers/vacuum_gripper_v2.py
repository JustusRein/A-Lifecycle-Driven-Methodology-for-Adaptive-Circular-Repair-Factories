...
class VacuumGripper(BaseGripper):
    ...
    def load_config(self) -> VacuumGripperConfig:
        ...
        # Parse sections independently
        basic_config = self._parse_basic_attributes(gripper_data)
        basic_config.pads = self._parse_pads_list(gripper_data)
    ...
    def _parse_pads_list(self, data: Dict[str, Any]) -> List[BaseSuctionPad]:
        """
        Parses the 'pads' list.
        Handles backward compatibility with legacy 'suction_cup' field.
        """
        pads_data = data.get("pads", [])

        if pads_data:
            # Modern Mode: List of pads defined explicitly
            return [BaseSuctionPad.from_config(p_data) for p_data in pads_data]
        else:
            # Legacy Mode: Create a single default pad from 'suction_cup'
            return [self._create_legacy_pad(data.get("suction_cup", {}))]

    def _create_legacy_pad(self, cup_data: Dict[str, Any]) -> BaseSuctionPad:
    ...
    def generate_safety_collision_mesh(
        ...
        """
        Generates the swept volume required for a safe approach.
        Aggregates safety volumes for ALL pads + the main body.
        """
        safety_margin = 1.0 + self.config.collision_margin
        parts = []

        # 1. Generate safety tubes for each active pad
        for pad in self.config.pads:
            parts.append(
                self._create_pad_volume(
                    pad, contact_point, surface_normal, approach_distance, safety_margin
                )
            )

        # 2. Generate safety tube for the main body
        parts.append(
    ...
    def _create_pad_volume(
        ...
        # 1. Calculate the world position of this specific pad based on TCP and Normal
        pad_world_start = self._calculate_pad_world_pos(pad, tcp_pos, normal)

        # 2. Get radius polymorphically (No if/else needed)
        safe_radius = pad.safety_radius * margin

        return gu.create_cylinder(
            radius=safe_radius,
            length=dist,
            start_point=pad_world_start,
    ...
    def _create_body_volume(
        ...
        """Creates the safety volume for the main body cylinder."""
        start_point = tcp + (
            normal * 0.03
        )  # Start slightly retracted to clear the cups
